                ...
                <form action="{{ route('dashboard.user.update', $item->id) }}" class="w-full" method="post" enctype="multipart/form-data">
                    ...
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            ...
                            <input type="text" value="{{ old('name') ?? $item->name }}" name="name" placeholder="User Name" class="block w-full bg-gray-200 text-gray-700 border-gray-200 rounded =py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                        </div>
                    </div>
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">Email</label>
                            <input type="email" value="{{ old('email') ?? $item->email }}" name="email" placeholder="User Email" class="block w-full bg-gray-200 text-gray-700 border-gray-200 rounded =py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                        </div>
                    </div>
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
